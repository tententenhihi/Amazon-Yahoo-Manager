...
const deleteUserData = async (userId) => {
    try {
        await YahooAccountModel.deleteMany({
            user_id: userId
        });
        await AuctionPublicSettingModel.deleteMany({
            user_id: userId
        });
        ...
        await ProductGlobalSettingModel.deleteMany({
            user_id: userId
        });
        await ProductInfomationDefaultModel.deleteMany({
            user_id: userId
        });
        await ProductYahooEndedModel.deleteMany({
            user_id: userId
        });
        await ProductYahooModel.deleteMany({
            user_id: userId
        });
        await RatingTemplateModel.deleteMany({
            user_id: userId
        });
        await TradeMessageTemplateModel.deleteMany({
            user_id: userId
        });
        await AsinModel.deleteMany({
            idUser: userId
        });
    } catch (error) {
...
class AdminController {
    ...
    static async createUser(req, res) {
        ...
        try {
            ...
            if (password != re_password) {
                return response.error400({
                    message: 'パスワード（確認）に一貫性がありません'
                });
            }
            let existUsers = await UserModel.findOne({
                email
            });
            if (!existUsers) {
                existUsers = await UserModel.findOne({
                    username
                });
            }
            if (existUsers) {
    ...
    static async unLockProxy(req, res) {
        ...
        try {
            ...
            if (_id === 'all') {
                let result = await ProxyModel.updateMany({
                    status: 'lock'
                }, {
                    status: 'live'
                });
                let listAccountYahoo = await YahooAccountModel.find({});
                let listProxyUsed = await ProxyModel.find({
                    status: 'used'
                });
                ...
                for (const proxyUsed of listProxyUsed) {
                    if (!listProxyAccountYahoo.includes(proxyUsed._id.toString())) {
                        await ProxyModel.findByIdAndUpdate(proxyUsed._id, {
                            status: 'live'
                        });
                    }
                }
                ...
                let result = await ProxyModel.findByIdAndUpdate(_id, {
                    status: 'live'
                });
                await YahooAccountModel.findOneAndUpdate({
                    proxy_id: _id
                }, {
                    proxy_id: null
                });
        ...
        try {
            ...
            for (let i = 0; i < listProxyOld.length; i++) {
                ...
                if (findProxy) {
                    ...
                    let yahooAccount = await YahooAccountModel.findOne({
                        proxy_id: proxyOld._id
                    });
                    if (yahooAccount) {
                        listAccountWithoutProxy.push(yahooAccount);
                    }
                    // Xóa proxy cũ
