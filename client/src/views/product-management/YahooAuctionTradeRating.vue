    ...
    <div class="box-content">
      <div class="px-30 pb-20 table-responsive">
        ...
        <div style="font-size:16px; margin: 5px 0">
          このオークションの取引について、落札者への満足度を採点してください。
          <br />
        ...
        <ValidationObserver tag="div" ref="formRating">
          ...
          <div class="form-group">
            ...
            >
              <option
                v-for="(rate, index) in RATING_LIST"
                :key="index"
                :value="rate.value"
                >{{ rate.display }}</option
              >
    ...
    async addRating() {
      ...
      if (result) {
        let payload = {
          ...
          rating: this.selectedTemplate
        };
        let res = await ProductYahooEndedApi.sendRating(payload);
        if (res && res.status === 200) {
          this.$swal.fire({
            icon: "success",
            title: "評価の追加が成功しました",
