  ...
  <div class="wrapper-content">
    ...
    <modal-component ref="modalNote">
      ...
      <template v-slot:button>
        <button class="btn btn-primary" @click="onSaveNote">保存</button>
...
export default {
  ...
  async mounted() {
    ...
    socket.on(this.$store.state.user._id + "-SELLING", (fetchedData) => {
        ...
        .fire({
          ...
          html:
            product.buyer_count > 0
              ? `<span style="color: red">このオークションには入札がありました。一度でも入札のあったオークション（入札を取り消した場合を含む）を取り消すと、550 円(税込)の出品取消システム利用料がかかります。
出品取消システム利用料のお支払いに同意の上、オークションを取り消す場合は、「取り消す」をクリックしてください。</span>`
              : `この製品を本当に削除しますか？`,
          icon: "warning",
