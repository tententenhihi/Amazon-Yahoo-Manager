      ...
      <div class="px-50 py-20">
        <div class="d-flex my-20">
          <input
            type="text"
            v-model="name"
            class="form-control col-8 col-lg-4 col-xl-3"
          />
        ...
        <div style="max-width: 600px; width:100%">
          <ul class="list-group">
            ...
            >
              ...
              >
                ...
                <label
                  :for="folder._id"
                  style="height: 36px; line-height: 36px; font-size: 14px"
                >
                ...
                <div class="group-button">
                  <button
                    ...
                    v-if="!folder.isEdit"
                    @click="onEditFolder(folder, index)"
                  >
                    編集 =
                  </button>
                  <template v-if="folder.isEdit">
                    <button
                      ...
                      @click="onSaveFolder(folder, index)"
                    >
                      保存する
                    </button>
                    <button
                      class="btn btn-outline-warning"
                      @click="onCancelEditFolder()"
                    >
                      キャンセル
                    </button>
                  </template>
          ...
          <div>
            <div v-if="folders.length">
              ...
              に
              <button class="btn btn-info mb-1" @click="moveProductToFolder">
                移動
              </button>
            </div>
            <button
...
export default {
  ...
  methods: {
    ...
    async addFolder() {
      let res = await FolderApi.create({
        name: this.name,
      ...
      if (this.selectedFolder.length) {
        ...
        if (res && res.status === 200) {
          ...
          for (let index = 0; index < this.selectedFolder.length; index++) {
            ...
            let findIndex = this.folders.findIndex(
